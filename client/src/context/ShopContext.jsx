...
const ShopContextProvider = ({ children }) => {
  ...
  const delivery_charges = 50 //50 rupees delivery charge
    ...
    try {
      ...
      if (data.success) {
        if (Array.isArray(data.products)) {
          setBooks(data.products.sort((a, b) => a.name.localeCompare(b.name)))
        } else {
          setBooks([])
        }
      } else {
  ...
  const logoutUser = async () => {
    try {
      ...
      if (data.success) {
        ...
        await fetchUser() // Re-fetch user status to ensure state is updated
        navigate('/')
      } else {
        toast.error(data.message)
      }
  ...
  const value = {
    ...
    setShowUserLogin,
    isAdmin,
    setIsAdmin,axios,fetchBooks,fetchUser,logoutUser
  };
