...
const CartTotal = () => {
  ...
  const placeOrder = async () => {
    try {
      ...
      if (method ==="COD") {
        const { data } = await axios.post("/api/order/cod", { items, address: selectedAddresses._id })
        if (data.success) {
        ...
        const { data } = await axios.post("/api/order/stripe", { items, address: selectedAddresses._id })
        if (data.success) {
      ...
      <div className="mb-5">
        <div className="my-5">
          ...
          <div className="relative flex justify-between items-start mt-2">
            ...
            <button onClick={() => setShowAddress(!showAddress)} className="text-secondary medium-14 hover:underline cursor-pointer">
              Change
            </button>
            {showAddress && (
        ...
        <hr className="border-gray-300 my-5" />
        <div className="my-6">
          <h4 className="h4 nb-5">Payment Method?</h4>
