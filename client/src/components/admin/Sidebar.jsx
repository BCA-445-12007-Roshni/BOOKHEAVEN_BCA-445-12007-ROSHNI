...
const Sidebar = () => {
  ...
  return (
    <div className='mx-auto max-w-[1550px] flex flex-col sm:flex-row'>
      {/* SIDEBAR */}
      <div className='max-sm:flexCenter max-sm:pb-3 bg-primary pb-3 m-2 sm:min-w-[20%] sm:min-h-[97vh] rounded-xl'>
        <div className='flex flex-col gap-y-6 max-sm-items-center sm:flex-col pt-4 sm:pt-14'>
          ...
          <Link to={"/admin"} className='bold-20 md:bold-24 uppercase font-paci lg:pl-[15%]'>
            Book<span className='text-secondary bold-28'>Heaven</span>
          </Link>
          <div className='flex sm:flex-col sm:gap-x-5 gap-y-8 sm:pt-10'>
            {navItems.map((link) => (
              <NavLink key={link.label} to={link.path} end={link.path === "/admin"} className={({ isActive }) => isActive
